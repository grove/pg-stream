...
| **Outbox relay latency bounded by poll interval.** Relays discover new outbox rows by polling; no push notification is sent after the outbox INSERT. Minimum relay latency equals the poll interval (`visibility_seconds`). | pg_trickle already uses `pgtrickle_wake` NOTIFY for internal scheduling; extending this to outbox writes was deferred to keep the v0.23.0 scope contained. | v0.24.0 relay: emit `pg_notify('pgtrickle_outbox_new', outbox_table_name)` after each outbox INSERT, allowing `pgtrickle-relay` to use `LISTEN/NOTIFY` for sub-second wake-up. |
...
| OUTBOX-6 | **Lifecycle & cascade.** `drop_stream_table()` cascades to outbox table + delta rows table + metadata. `alter_stream_table()` errors if column set changed while outbox enabled. `outbox_status()` monitoring function (includes `claim_check_pending_count` field). `outbox_rows_consumed(stream_table TEXT, outbox_id BIGINT)` SQL function: called by relay after cursor consumption to record per-group completion in `pgt_consumer_claim_check_acks`; idempotent. 5 Part-A GUCs (`outbox_enabled`, `outbox_retention_hours`, `outbox_drain_batch_size`, `outbox_inline_threshold_rows`, `outbox_drain_interval_seconds`) + 4 Part-B GUCs (`consumer_dead_threshold_hours`, `consumer_stale_offset_threshold_days`, `consumer_cleanup_enabled`, `outbox_force_retention`). | 0.5d | [PLAN_TRANSACTIONAL_OUTBOX_HELPER.md](plans/patterns/PLAN_TRANSACTIONAL_OUTBOX_HELPER.md) §A.7–A.8 |
