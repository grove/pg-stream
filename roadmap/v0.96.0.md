...
Provide `pgtrickle_admin`, `pgtrickle_operator`, and `pgtrickle_reader` as a
user-facing mapping onto the ACL and ownership model from v0.84.0. PostgreSQL
reserves names beginning with `pg_`, so the separator is omitted. The roles do
not create a second authorization layer. Positive and negative tests cover the
diagnostic and refresh boundaries, including `SECURITY DEFINER` behavior.
