...
centroids *incrementally* using `vector_avg` and `vector_sum` algebraic
aggregates that apply element-wise arithmetic in the L2 norm domain.

v0.37.0 adds `vector_avg` and `vector_sum` aggregate operators to the DVM
engine that are compatible with pgvector 0.7+ HNSW and IVF indexes. A stream
table over a document-embeddings table can maintain a live cluster centroid
that is always within one refresh cycle of the latest embeddings, without
recomputing the full set. The DVM distance-operator classifier is also
codified as a stable contract: any `<->`, `<=>`, `<#>` operators in a stream
table are explicitly documented to fall back to FULL refresh.
