...
> **Renumbered:** previously v0.94.0.

## Theme

Finish the monitoring contract, run release assurance on one candidate commit,
and prove that each supported package can install and upgrade. The resulting
evidence becomes the baseline for the remaining pre-1.0 work.
...
- **Upgrade matrix:** Run every pair in the bounded v1.0 support manifest from
  ...
  v0.40.0 through the latest v0.105.x release on PostgreSQL 18.
...
delay the evidence baseline when the artifact, smoke tests, and publication
automation are ready.
...
- [ ] Publication automation is ready. External repository acceptance is
      tracked separately
- [ ] The release evidence manifest records the candidate commit, artifact
      digests, executed suites, skipped suites, and retained logs
