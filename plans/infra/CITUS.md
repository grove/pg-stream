...
- Store placement in `DtDependency` struct in `src/catalog.rs`
...
For `dt_placement = 'distributed'`, replace the MERGE statement in `src/refresh.rs` (L165-L175) with two Citus-compatible statements:
...
pub struct CachedMergeTemplate {
    ...
    pub dt_placement: String,
}
```

**P5.5: Update IVM output for distributed mode**

`src/dvm/mod.rs` must emit the two-statement form when `dt_placement = 'distributed'`. The delta CTE chain itself is unchanged — only the final apply statement differs.
...
```sql
CREATE TABLE pg_stream.dt_locks (
...
```sql
INSERT INTO pg_stream.dt_locks (pgs_id, locked_by, locked_at)
...
```sql
DELETE FROM pg_stream.dt_locks WHERE pgs_id = $1 AND locked_by = pg_backend_pid()
```
...
3. Add `dt_placement TEXT DEFAULT 'local'` column to `pg_stream.pgs_stream_tables`
4. Create `pg_stream.change_seq` sequence
5. Create `pg_stream.dt_locks` table
...
| `src/catalog.rs` | 1, 2 | SourceIdentifier, placement in DtDependency |
